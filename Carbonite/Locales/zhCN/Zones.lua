if ( GetLocale() ~= "zhCN" ) then
	return
end

local L = LibStub("AceLocale-3.0"):NewLocale("Carbonite", "zhCN")
if not L then return end

L["A Brewing Storm"] = "酝酿风暴"
L["A Little Patience"] = "王者的耐心"
L["Abyssal Depths"] = "无底海渊"
L["Ahn'kahet: The Old Kingdom"] = "安卡赫特：古代王国"
L["Temple of Ahn'Qiraj"] = "安其拉神殿"		--	need retranslation (Ahn'Qiraj = 安其拉)
L["Ahn'Qiraj: The Fallen Kingdom"] = "安其拉：堕落王国"
L["Alterac Mountains"] = "奥特兰克山脉"
L["Alterac Valley"] = "奥特兰克山谷"
L["Ammen Vale"] = "埃门谷"
L["Arathi Basin"] = "阿拉希盆地"
L["Arathi Highlands"] = "阿拉希高地"
L["Arena of Annihilation"] = "破军比武场"
L["Ashenvale"] = "灰谷"
L["Ashran"] = "阿什兰"
L["Assault on Zan'vess"] = "突袭扎尼维斯"
L["Auchenai Crypts"] = "奥金尼地穴"
L["Auchindoun"] = "奥金顿"
L["Azjol-Nerub"] = "艾卓-尼鲁布"
L["Azshara"] = "艾萨拉"
L["Azuremyst Isle"] = "秘蓝岛"
L["Badlands"] = "荒芜之地"
L["Baradin Hold"] = "巴拉丁监狱"
L["Theramore's Fall (A)"] = "塞拉摩的沦陷"		--	Replaces Battle of Theramore (塞拉摩之战)
L["Theramore's Fall (H)"] = "塞拉摩的沦陷"		--	Replaces Battle of Theramore (塞拉摩之战)
L["Battle on the High Seas"] = "公海激战"
L["Black Temple Scenario"] = "黑暗神殿场景战役"
L["Black Temple"] = "黑暗神殿"
L["Blackfathom Deeps"] = "黑暗深渊"
L["Blackrock Caverns"] = "黑石岩窟"
L["Blackrock Depths"] = "黑石深渊"
L["Blackrock Foundry"] = "黑石铸造厂"
L["Blackrock Spire"] = "黑石塔"
L["Blackwing Descent"] = "黑翼血环"
L["Blackwing Lair"] = "黑翼之巢"
L["Blade's Edge Arena"] = "刀锋山竞技场"
L["Blade's Edge Mountains"] = "刀锋山"
L["Blasted Lands"] = "诅咒之地"
L["Blood in the Snow"] = "雪山血战"
L["Bloodmaul Slag Mines"] = "血槌炉渣矿井"
L["Bloodmyst Isle"] = "秘血岛"
L["Borean Tundra"] = "北风苔原"
L["Brewmoon Festival"] = "酿月祭"
L["Burning Steppes"] = "燃烧平原"
L["Camp Narache"] = "纳拉其营地"
L["Celestial Tournament"] = "天神比武大会"
L["Coldridge Valley"] = "寒脊山谷"
L["Crypt of Forgotten Kings"] = "遗忘之王古墓"
L["Crystalsong Forest"] = "晶歌森林"
L["Dagger in the Dark"] = "黑暗中的匕首"
L["Dalaran Underbelly"] = "达拉然下水道"
L["Dalaran"] = "达拉然"
L["Dark Heart of Pandaria"] = "潘达利亚的黑暗之心"
L["Darkmoon Island"] = "暗月岛"
L["Darkshore"] = "黑海岸"
L["Darnassus"] = "达纳苏斯"
L["Deadwind Pass"] = "逆风小径"
L["Deathknell"] = "丧钟镇"
L["Deepholm"] = "深岩之洲"
L["Deeprun Tram"] = "矿道地铁"
L["Deepwind Gorge"] = "深风峡谷"
L["Desolace"] = "凄凉之地"
L["Dire Maul"] = "厄运之槌"
L["Domination Point"] = "统御岗哨"
L["Draenor"] = "德拉诺"
L["Dragon Soul"] = "巨龙之魂"
L["Dragonblight"] = "龙骨荒野"
L["Drak'Tharon Keep"] = "达克萨隆要塞"
L["Dread Wastes"] = "恐惧废土"
L["Dun Morogh"] = "丹莫罗"
L["Durotar"] = "杜隆塔尔"
L["Duskwood"] = "暮色森林"
L["Dustwallow Marsh"] = "尘泥沼泽"
L["Eastern Kingdoms"] = "东部王国"
L["Eastern Plaguelands"] = "东瘟疫之地"
L["Echo Isles"] = "回音群岛"
L["Elwynn Forest"] = "艾尔文森林"
L["End Time"] = "时光之末"
L["Eversong Woods"] = "永歌森林"
L["Eye of the Storm"] = "风暴之眼"
L["Felwood"] = "费伍德森林"
L["Feralas"] = "菲拉斯"
L["Firelands"] = "火焰之地"
L["Frostfire Ridge"] = "霜火岭"
L["Frostwall"] = "霜寒晶壁"
L["Gate of the Setting Sun"] = "残阳关"
L["Ghostlands"] = "幽魂之地"
L["Gilneas City"] = "吉尔尼斯城"
L["Gilneas"] = "吉尔尼斯"
L["Gnomeregan"] = "诺莫瑞根"
L["Gorgrond"] = "戈尔隆德"
L["Greenstone Village"] = "绿石村"
L["Grim Batol"] = "格瑞姆巴托"
L["Grimrail Depot"] = "恐轨车站"
L["Grizzly Hills"] = "灰熊丘陵"
L["Gruul's Lair"] = "格鲁尔的巢穴"
L["Gundrak"] = "古达克"
L["Halls of Lightning"] = "闪电大厅"
L["Halls of Origination"] = "起源大厅"
L["Halls of Reflection"] = "映像大厅"
L["Halls of Stone"] = "岩石大厅"
L["Heart of Fear"] = "恐惧之心"
L["Hellfire Peninsula"] = "地狱火半岛"
L["Hellfire Ramparts"] = "地狱火城墙"
L["Highmaul"] = "悬槌堡"
L["Hillsbrad Foothills"] = "希尔斯布莱德丘陵"
L["Hour of Twilight"] = "暮光审判"
L["Howling Fjord"] = "嚎风峡湾"
L["Hrothgar's Landing"] = "洛斯加尔登陆点"
L["Hyjal Summit"] = "海加尔峰"
L["Icecrown Citadel"] = "冰冠堡垒"
L["Icecrown"] = "冰冠冰川"
L["Iron Docks"] = "钢铁码头"
L["Ironforge"] = "铁炉堡"
L["Isle of Conquest"] = "征服之岛"
L["Isle of Giants"] = "巨兽岛"
L["Isle of Quel'Danas"] = "奎尔丹纳斯岛"
L["Isle of Thunder Scenario"] = "雷神岛场景战役"
L["Isle of Thunder"] = "雷神岛"
L["Kalimdor"] = "卡利姆多"
L["Karazhan"] = "卡拉赞"
L["Kelp'thar Forest"] = "柯尔普萨之森"
L["Kezan"] = "科赞"
L["Krasarang Wilds"] = "卡桑琅丛林"
L["Kun-Lai Summit"] = "昆莱山"
L["Lion's Landing"] = "雄狮港"
L["Loch Modan"] = "洛克莫丹"
L["Lost City of the Tol'vir"] = "托维尔失落之城"
L["Lunarfall"] = "坠落之月"
L["Magisters' Terrace"] = "魔导师平台"
L["Magtheridon's Lair"] = "玛瑟里顿的巢穴"
L["Mana-Tombs"] = "法力陵墓"
L["Maraudon"] = "玛拉顿"
L["Mogu'shan Palace"] = "魔古山宫殿"
L["Mogu'shan Vaults"] = "魔古山宝库"
L["Molten Front"] = "熔火前线"
L["Moonglade"] = "月光林地"
L["Mount Hyjal"] = "海加尔山"
L["Mulgore"] = "莫高雷"
L["Nagrand Arena"] = "纳格兰竞技场"
L["Nagrand"] = "纳格兰"
L["Naxxramas"] = "纳克萨玛斯"
L["Netherstorm"] = "虚空风暴"
L["New Tinkertown"] = "新工匠镇"
L["Northern Barrens"] = "北贫瘠之地"
L["Northern Stranglethorn"] = "北荆棘谷"
L["Northrend"] = "诺森德"
L["Northshire"] = "北郡"
L["Old Hillsbrad Foothills"] = "旧希尔斯布莱德丘陵"
L["Onyxia's Lair"] = "奥妮克希亚的巢穴"
L["Orgrimmar"] = "奥格瑞玛"
L["Outland"] = "外域"
L["Pandaria" ] = "潘达利亚"
L["Pit of Saron"] = "萨隆矿坑"
L["Plaguelands: The Scarlet Enclave"] = "东瘟疫之地：血色领地"
L["Ragefire Chasm"] = "怒焰裂谷"
L["Razorfen Downs"] = "剃刀高地"
L["Razorfen Kraul"] = "剃刀沼泽"
L["Redridge Mountains"] = "赤脊山"
L["Ruins of Ahn'Qiraj"] = "安其拉废墟"
L["Ruins of Gilneas City"] = "吉尔尼斯城废墟"
L["Ruins of Gilneas"] = "吉尔尼斯废墟"
L["Ruins of Lordaeron"] = "洛丹伦废墟"
L["Scarlet Halls"] = "血色大厅"
L["Scarlet Monastery"] = "血色修道院"
L["Scholomance"] = "通灵学院"
L["Searing Gorge"] = "灼热峡谷"
L["Serpentshrine Cavern"] = "毒蛇神殿"
L["Sethekk Halls"] = "塞泰克大厅"
L["Shado-Pan Monastery"] = "影踪禅院"
L["Shadow Labyrinth"] = "暗影迷宫"
L["Shadowfang Keep"] = "影牙城堡"
L["Shadowglen"] = "幽影谷"
L["Shadowmoon Burial Grounds"] = "影月墓地"
L["Shadowmoon Valley"] = "影月谷"
L["Shattrath City"] = "沙塔斯城"
L["Shimmering Expanse"] = "烁光海床"
L["Sholazar Basin"] = "索拉查盆地"
L["Shrine of Seven Stars"] = "七星殿"
L["Shrine of Two Moons"] = "双月殿"
L["Siege of Niuzao Temple"] = "围攻砮皂寺"
L["Siege of Orgrimmar"] = "决战奥格瑞玛"
L["Silithus"] = "希利苏斯"
L["Silvermoon City"] = "银月城"
L["Silverpine Forest"] = "银松森林"
L["Silvershard Mines"] = "碎银矿脉"
L["Skyreach"] = "通天峰"
L["Southern Barrens"] = "南贫瘠之地"
L["Spires of Arak"] = "阿兰卡峰林"
L["Stonetalon Mountains"] = "石爪山脉"
L["Stormshield"] = "暴风之盾"
L["Stormstout Brewery"] = "风暴烈酒酿造厂"
L["Stormwind City"] = "暴风城"
L["The Stockade"] = "监狱"		--	Replaces Stormwind Stockade (暴风城监狱)
L["Strand of the Ancients"] = "远古海滩"
L["Stranglethorn Vale"] = "荆棘谷"
L["Stratholme"] = "斯坦索姆"
L["Sunstrider Isle"] = "逐日岛"
L["Sunwell Plateau"] = "太阳之井高地"
L["Swamp of Sorrows"] = "悲伤沼泽"
L["Talador"] = "塔拉多"
L["Tanaan Jungle"] = "塔纳安丛林"
L["Tanaris"] = "塔纳利斯"
L["Teldrassil"] = "泰达希尔"
L["The Eye"] = "风暴要塞"				--	Replaces Tempest Keep (风暴要塞)
L["Temple of Kotmogu"] = "寇魔古寺"
L["Temple of the Jade Serpent"] = "青龙寺"
L["Terokkar Forest"] = "泰罗卡森林"
L["Terrace of Endless Spring"] = "永春台"
L["The Arcatraz"] = "禁魔监狱"
L["The Bastion of Twilight"] = "暮光堡垒"
L["The Battle for Gilneas"] = "吉尔尼斯城"
L["The Black Morass"] = "黑色沼泽"
L["The Blood Furnace"] = "鲜血熔炉"
L["The Botanica"] = "生态船"
L["The Cape of Stranglethorn"] = "荆棘谷海角"
L["The Culling of Stratholme"] = "净化斯坦索姆"
L["The Deadmines"] = "死亡矿井"
L["The Everbloom"] = "永茂林地"
L["The Exodar"] = "埃索达"
L["The Eye of Eternity"] = "永恒之眼"
L["The Forge of Souls"] = "灵魂洪炉"
L["The Hinterlands"] = "辛特兰"
L["The Jade Forest"] = "翡翠林"
L["The Lost Isles"] = "失落群岛"
L["The Maelstrom"] = "大漩涡"
L["The Mechanar"] = "能源舰"
L["Molten Core"] = "熔火之心"
L["The Nexus"] = "魔枢"
L["The Obsidian Sanctum"] = "黑曜石圣殿"
L["The Oculus"] = "魔环"
L["The Ruby Sanctum"] = "红玉圣殿"
L["The Secrets of Ragefire"] = "怒焰之谜"
L["The Shattered Halls"] = "破碎大厅"
L["The Slave Pens"] = "奴隶围栏"
L["The Steamvault"] = "蒸汽地窟"
L["The Stonecore"] = "巨石之核"
L["The Storm Peaks"] = "风暴峭壁"
L["The Temple of Atal'Hakkar"] = "阿塔哈卡神庙"
L["The Underbog"] = "幽暗沼泽"
L["The Veiled Stair"] = "雾纱栈道"
L["The Violet Hold"] = "紫罗兰监狱"
L["The Vortex Pinnacle"] = "旋云之巅"
L["The Wandering Isle"] = "迷踪岛"
L["Thousand Needles"] = "千针石林"
L["Throne of the Four Winds"] = "风神王座"
L["Throne of the Tides"] = "潮汐王座"
L["Throne of Thunder"] = "雷电王座"
L["Thunder Bluff"] = "雷霆崖"
L["Thunder King's Citadel"] = "雷电堡"
L["Tigers Peak Arena"] = "虎踞峰竞技场"
L["Timeless Isle"] = "永恒岛"
L["Tirisfal Glades"] = "提瑞斯法林地"
L["Tol Barad Peninsula"] = "托尔巴拉德半岛"
L["Tol Barad"] = "托尔巴拉德"
L["Tol'vir Proving Grounds"] = "托维尔试炼场"
L["Townlong Steppes"] = "螳螂高原"
L["Trial of the Champion"] = "冠军的试炼"
L["Trial of the Crusader"] = "十字军的试炼"
L["Twilight Highlands"] = "暮光高地"
L["Twin Peaks"] = "双子峰"
L["Uldaman"] = "奥达曼"
L["Ulduar"] = "奥杜尔"
L["Uldum"] = "奥丹姆"
L["Undercity"] = "幽暗城"
L["Unga Ingoo"] = "盎迦猴岛"
L["Un'Goro Crater"] = "安戈洛环形山"
L["Unknown Zone"] = "未知区域"
L["Upper Blackrock Spire"] = "黑石塔上层"
L["Utgarde Keep"] = "乌特加德城堡"
L["Utgarde Pinnacle"] = "乌特加德之巅"
L["Vale of Eternal Blossoms"] = "锦绣谷"
L["Valley of the Four Winds"] = "四风谷"
L["Valley of Trials"] = "试炼谷"
L["Vashj'ir"] = "瓦丝琪尔"
L["Vault of Archavon"] = "阿尔卡冯的宝库"
L["Wailing Caverns"] = "哀嚎洞穴"
L["Warsong Gulch"] = "战歌峡谷"
L["Warspear"] = "战争之矛"
L["Well of Eternity"] = "永恒之井"
L["Western Plaguelands"] = "西瘟疫之地"
L["Westfall"] = "西部荒野"
L["Wetlands"] = "湿地"
L["Wintergrasp"] = "冬拥湖"
L["Winterspring"] = "冬泉谷"
L["Zangarmarsh"] = "赞加沼泽"
L["Zul'Aman"] = "祖阿曼"
L["Zul'Drak"] = "祖达克"
L["Zul'Farrak"] = "祖尔法拉克"
L["Zul'Gurub"] = "祖尔格拉布"

L["Cantrips & Crows"] = "咒语和乌鸦旅店"
L["Circle of Wills"] = "意志竞技场"
L["The Black Market"] = "黑市"
L["The Underbelly"] = "屠宰场"
